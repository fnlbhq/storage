...
func setup() {
	...
	Put(bucket, updateTemp, testData())
}
...
func TestProcessFile(t *testing.T) {
	...
	ProcessFile(bucket, filename, func(data []byte) error {
		testField = unmarshal(data)[1]["Series"].(string)
		return nil
	})
...
func TestProcessAndUpdateFile(t *testing.T) {
	...
	assert.Nil(t, err)
	updatedObjectBytes, _ := Get(bucket, updateTemp)
...
//	zippedBytes, _ := lib.Zip(files)
//	Put(bucket, zipTemp, zippedBytes)
//	result, _ := Get(bucket, zipTemp)
//	unZipped, _ := lib.Decompress(result)
